...
function addProductToCart(productId) {
  ...
  if (cartItem) {
    cartItem.quantity += 1; // CASE 1: Increment quantity if product already exists in the cart
  } else {
    SHOPPING_CART.push({ id: productId, quantity: 1 }); // CASE 2: Add product with quantity 1 if it does not exist
  }
}
...
function removeProductFromCart(productId) {
  // Write your code here
  const cartIndex = SHOPPING_CART.findIndex(item => item.id === productId);
  if (cartIndex !== -1) {
    const cartItem = SHOPPING_CART[cartIndex];
    if (cartItem.quantity > 1) {
      cartItem.quantity -= 1; // CASE 1: Decrement quantity if it is >=2
    } else {
      SHOPPING_CART.splice(cartIndex, 1); // CASE 2: Remove product if quantity is 1
    }
  }
}
